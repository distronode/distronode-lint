...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            [
                ("WARNING  Listing 1 violation(s) that are fatal", False),
                ("INFO     Set DISTRONODE_LIBRARY=", False),
                ("DEBUG ", True),
            ],
            id="v",
        ),
...
def test_verbosity(
    ...
    if verbosity:
        result = run_distronode_lint(verbosity, str(fakerole), cwd=project_path)
    else:
        result = run_distronode_lint(str(fakerole), cwd=project_path)
