...
import distronodelint.__main__ as main
from distronodelint.app import App
from distronodelint.file_utils import Lintable

# noinspection PyProtectedMember
from distronodelint.runner import LintResult, get_matches
...
@pytest.mark.parametrize(
    ("playbook_str", "matches_count", "transformed", "is_owned_by_distronode"),
    (
...
def test_transformer(  # pylint: disable=too-many-arguments
    ...
    """
    # test ability to detect is_owned_by_distronode
...
def test_pruned_err_after_fix(monkeypatch: pytest.MonkeyPatch, tmpdir: Path) -> None:
    ...
    monkeypatch.setattr("sys.argv", ["distronode-lint", str(dest), "--fix=all"])
    ...
    monkeypatch.setattr("distronodelint.__main__.fix", test_fix)
    monkeypatch.setattr("distronodelint.app.App", TestApp)
