...
from distronodelint.config import get_version_warning
...
def test_call_from_outside_venv(expected_warning: bool) -> None:
    ...
    proc = subprocess.run(
        [str(py_path / "distronode-lint"), "--version"],
        check=False,
        capture_output=True,
        text=True,
        env=env,
...
def test_get_version_warning(
    ...
    # overwrite distronode-lint version
    mocker.patch("distronodelint.config.__version__", "1.2.3")
    # overwrite install method to custom one. This one will increase msg line count
    # to easily detect unwanted call to it.
    mocker.patch("distronodelint.config.guess_install_method", return_value="\n")
...
def test_nodeps(lintable: str) -> None:
    """Asserts ability to be called w/ or w/o venv activation."""
    env = os.environ.copy()
    env["DISTRONODE_LINT_NODEPS"] = "1"
    py_path = Path(sys.executable).parent
    proc = subprocess.run(
        [str(py_path / "distronode-lint"), lintable],
        check=False,
        capture_output=True,
        text=True,
        env=env,
...
def test_broken_distronode_cfg() -> None:
    """Asserts behavior when encountering broken distronode.cfg files."""
    py_path = Path(sys.executable).parent
    proc = subprocess.run(
        [str(py_path / "distronode-lint"), "--version"],
        check=False,
        capture_output=True,
        text=True,
        cwd="test/fixtures/broken-distronode.cfg",
    )
