...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(["-t", "skip_distronode_lint"], "test/fixtures/tags.yml", id="6"),
...
def test_config_can_be_overridden(base_arguments: list[str]) -> None:
    ...
    assert [*no_override.tags, "skip_distronode_lint"] == overridden.tags
...
def test_different_config_file(base_arguments: list[str]) -> None:
    ...
    diff_config = cli.get_config(
        [*base_arguments, "-c", "test/fixtures/distronode-config.yml"],
    )
...
def test_expand_path_user_and_vars_config_file(base_arguments: list[str]) -> None:
    ...
    assert str(config1.exclude_paths[1]) == os.path.expandvars("$HOME/.distronode/roles")
    ...
    assert str(config2.exclude_paths[0]) == os.path.realpath(
        os.path.expanduser("~/.distronode/roles"),  # noqa: PTH111
    )
    assert str(config2.exclude_paths[1]) == os.path.realpath(
        os.path.expandvars("$HOME/.distronode/roles"),
    )
...
@pytest.mark.parametrize(
    "config_file",
    (
        pytest.param("test/fixtures/distronode-config-invalid.yml", id="invalid"),
        pytest.param("/dev/null/distronode-config-missing.yml", id="missing"),
    ),
)
