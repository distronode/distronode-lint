...
from distronodelint.utils import task_in_list
...
def test_nested_items_path() -> None:
    ...
    assert list(distronodelint.yaml_utils.nested_items_path(data)) == items
...
def test_nested_items_path_raises_typeerror(invalid_data_input: Any) -> None:
    ...
    with pytest.raises(TypeError, match=r"Expected a dict or a list.*"):
        list(distronodelint.yaml_utils.nested_items_path(invalid_data_input))
...
@pytest.mark.parametrize(
    ...
    (
        ...
        pytest.param(
            ...
            2,
            0,
            distronodelint.yaml_utils.FormattedEmitter,
            _DOUBLE_QUOTE_WITHOUT_INDENTS,
...
def test_fmt(before: str, after: str, version: tuple[int, int] | None) -> None:
    ...
    yaml = distronodelint.yaml_utils.FormattedYAML(version=version)
...
def test_formatted_yaml_loader_dumper(
    ...
    config = distronodelint.yaml_utils.load_yamllint_config()
...
def fixture_ruamel_data(lintable: Lintable) -> CommentedMap | CommentedSeq:
    ...
    yaml = distronodelint.yaml_utils.FormattedYAML()
...
def test_get_path_to_play(
    ...
    path_to_play = distronodelint.yaml_utils.get_path_to_play(
        lintable,
        lineno,
        ruamel_data,
    )
...
def test_get_path_to_task(
    ...
    path_to_task = distronodelint.yaml_utils.get_path_to_task(
        lintable,
        lineno,
        ruamel_data,
    )
...
def test_get_path_to_play_raises_value_error_for_bad_lineno(
    ...
    with pytest.raises(
        ...
        distronodelint.yaml_utils.get_path_to_play(lintable, lineno, ruamel_data)
...
def test_get_path_to_task_raises_value_error_for_bad_lineno(
    ...
    with pytest.raises(
        ...
        distronodelint.yaml_utils.get_path_to_task(lintable, lineno, ruamel_data)
...
def test_deannotate(
    ...
    assert distronodelint.yaml_utils.deannotate(before) == after
