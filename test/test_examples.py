...
def test_custom_kinds() -> None:
    ...
    result = run_distronode_lint("-vv", "--offline", "examples/other/")
    assert result.returncode == 0
    # .yaml-too is not a recognized extension and unless is manually defined
    # in our distronode-lint config, the test would not identify it as yaml file.
...
def test_bug_3216(capsys: pytest.CaptureFixture[str]) -> None:
    """Check that we hide distronode-core originating warning about fallback on unique filter."""
    ...
    assert result.returncode == 0
    warn_msg = "Falling back to Distronode unique filter"
