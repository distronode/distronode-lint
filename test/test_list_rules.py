...
def test_list_rules_includes_opt_in_rules(project_path: Path) -> None:
    ...
    result_list_rules = run_distronode_lint("-L", fakerole, cwd=project_path)
...
def test_list_rules_includes_autofix() -> None:
    ...
    result_list_rules = run_distronode_lint("--list-rules")
...
def test_list_rules_with_format_option(
    ...
    result_list_rules = run_distronode_lint(
        "-f",
        format_string,
        "-L",
        fakerole,
...
def test_list_tags_includes_opt_in_rules(project_path: Path) -> None:
    ...
    result_list_tags = run_distronode_lint("-L", str(fakerole), cwd=project_path)
