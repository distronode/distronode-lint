...
def test_run_single_role_path_no_trailing_slash_module(local_test_dir: Path) -> None:
    ...
    result = run_distronode_lint(role_path, cwd=cwd)
...
def test_run_single_role_path_no_trailing_slash_script(local_test_dir: Path) -> None:
    ...
    result = run_distronode_lint(role_path, cwd=cwd, executable="distronode-lint")
    assert "Use shell only when shell functionality is required" in result.stdout
...
def test_run_playbook(local_test_dir: Path) -> None:
    """Call distronode-lint the way molecule does."""
    cwd = local_test_dir / "roles" / "test-role"
    lintable = "molecule/default/include-import-role.yml"
    role_path = str(Path(cwd).parent.resolve())

    env = os.environ.copy()
    env["DISTRONODE_ROLES_PATH"] = role_path
    env["NO_COLOR"] = "1"

    result = run_distronode_lint("-f", "pep8", lintable, cwd=cwd, env=env)
...
def test_run_role_name_invalid(
    ...
    result = run_distronode_lint(*args, role_path, cwd=cwd)
...
def test_run_role_name_with_prefix(local_test_dir: Path) -> None:
    ...
    cwd = local_test_dir
    role_path = "roles/distronode-role-foo"

    result = run_distronode_lint("-v", role_path, cwd=cwd)
...
def test_run_single_role_path_with_roles_path_env(local_test_dir: Path) -> None:
    """Test for role name collision with DISTRONODE_ROLES_PATH.

    Test if distronode-lint chooses the role in the current directory when the role
    specified as parameter exists in the current directory and the DISTRONODE_ROLES_PATH.
    """
    cwd = local_test_dir
    role_path = "roles/test-role"

    env = os.environ.copy()
    env["DISTRONODE_ROLES_PATH"] = os.path.realpath((cwd / "../examples/roles").resolve())

    result = run_distronode_lint(role_path, cwd=cwd, env=env)
    assert "Use shell only when shell functionality is required" in result.stdout
...
def test_run_playbook_github(result: bool, env: dict[str, str]) -> None:
    """Call distronode-lint simulating GitHub Actions environment."""
    ...
    result_gh = run_distronode_lint(role_path, cwd=cwd, env=env)
