...
def test_get_distronode_syntax_check_matches(
    ...
    assert (
        result[0].message
        == "conflicting action statements: distronode.builtin.debug, distronode.builtin.command"
    )
    # We internally convert absolute paths returned by distronode into paths
