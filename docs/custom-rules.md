...
Define and use your own sets of rules with Distronode-lint.
...
class DeprecatedVariableRule(DistronodeLintRule):
...
Distronode-lint automatically loads and enables custom rules in Python packages
from the _custom_ subdirectory. This subdirectory is part of the Distronode-lint
installation directory, for example:

`/usr/lib/python3.8/site-packages/distronodelint/rules/custom/`
...
2. Configure the \[options\] section of the `setup.cfg` of your custom rules
   ...
   [options]
   packages =
       distronodelint.rules.custom.<your_custom_rules_subdir>
   package_dir =
       distronodelint.rules.custom.<your_custom_rules_subdir> = <your_rules_source_code_subdir>
   ```

3. Install the Python package into
   `<distronode_lint_custom_rules_dir>/custom/<your_custom_rules_subdir>/`.
