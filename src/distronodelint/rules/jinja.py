...
import black
import jinja2
from distronode.errors import DistronodeError, DistronodeFilterError, DistronodeParserError
...
from distronodelint.errors import RuleMatchTransformMeta
...
from distronodelint.utils import parse_yaml_from_file, template
from distronodelint.yaml_utils import deannotate, nested_items_path
...
ignored_re = re.compile(
    "|".join(  # noqa: FLY002
        [
            ...
            r"is undefined",
            r"Unrecognized type <<class 'distronode.template.DistronodeUndefined'>> for (.*) filter <value>$",
            # https://github.com/distronode/distronode-lint/issues/3155
...
class JinjaRule(DistronodeLintRule, TransformMixin):
    ...
    _distronode_error_re = re.compile(
    ...
    def matchtask(
        ...
        try:
            for key, v, path in nested_items_path(
                task,
                ignored_keys=("block", "distronode.builtin.block", "distronode.legacy.block"),
            ):
                if isinstance(v, str):
                    try:
                        template(
                    ...
                    # ValueError RepresenterError
                        ...
                        match = self._distronode_error_re.match(
                        ...
                        if ignored_re.search(orig_exc_message) or isinstance(
                            ...
                            # distronode@devel (2.14) new behavior:
                            ...
                            if error.startswith(
                                ...
                                _logger.debug("Ignored DistronodeError: %s", exc)
                                bypass = True
                            else:
                                bypass = False
                        ...
                        # DistronodeError: template error while templating string: expected token ':', got '}'. String: {{ {{ '1' }} }}
                        # DistronodeError: template error while templating string: unable to locate collection distronode.netcommon. String: Foo {{ buildset_registry.host | ipwrap }}
                        if not bypass:
                            result.append(
    ...
    def matchyaml(self, file: Lintable) -> list[MatchError]:
        ...
        if str(file.kind) == "vars":
            ...
            for key, v, _path in nested_items_path(data):
                if isinstance(v, DistronodeUnicode):
                    reformatted, details, tag = self.check_whitespace(
                        v,
                        key=key,
                        lintable=file,
...
if "pytest" in sys.modules:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            # https://github.com/distronode/distronode-lint/pull/3057
    ...
    @mock.patch.dict(os.environ, {"DISTRONODE_LINT_WRITE_TMP": "1"}, clear=True)
