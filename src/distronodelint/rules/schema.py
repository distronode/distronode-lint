...
from distronodelint.schemas.__main__ import JSON_SCHEMAS
from distronodelint.schemas.main import validate_file_schema
...
class ValidateSchemaRule(DistronodeLintRule):
    ...
    _ids = {
        "schema[distronode-lint-config]": "",
        "schema[distronode-navigator-config]": "",
        "schema[changelog]": "",
...
if "pytest" in sys.modules:
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            pytest.param(
                ".distronode-lint",
                "distronode-lint-config",
                [],
                id="distronode-lint-config",
            ),
            pytest.param(
                "examples/.config/distronode-lint.yml",
                "distronode-lint-config",
                [],
                id="distronode-lint-config2",
            ),
            pytest.param(
                "examples/broken/.distronode-lint",
                "distronode-lint-config",
                [
                    r".*Additional properties are not allowed \('foo' was unexpected\).*https://",
                ],
                id="distronode-lint-config-broken",
            ),
            pytest.param(
                "examples/distronode-navigator.yml",
                "distronode-navigator-config",
                [],
                id="distronode-navigator-config",
            ),
            pytest.param(
                "examples/broken/distronode-navigator.yml",
                "distronode-navigator-config",
                [
                    r".*Additional properties are not allowed \('distronode' was unexpected\).*https://",
                ],
                id="distronode-navigator-config-broken",
            ),
            pytest.param(
    ...
    def test_schema(
        ...
        """Validate parsing of distronode output."""
