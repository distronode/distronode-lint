...
class FQCNBuiltinsRule(DistronodeLintRule, TransformMixin):
    ...
    def matchtask(
        ...
        if module != self.module_aliases[module]:
            ...
            if module_alias.startswith("distronode.builtin"):
                ...
                if module != legacy_module:
                    if module == "distronode.builtin.include":
                        message = f"Avoid deprecated module ({module})"
                        details = "Use `distronode.builtin.include_task` or `distronode.builtin.import_tasks` instead."
                    else:
                ...
                # https://github.com/distronode-community/community-topics/issues/147
    ...
    def transform(
        ...
        if match.tag in self.ids():
            ...
            # Unfortunately, a lot of data about Distronode content gets lost here, you only get a simple dict.
            ...
            if match.tag == "fqcn[action-core]":
                ...
                # This will always replace builtin modules with "distronode.builtin" versions, not "distronode.legacy".
                # The latter is technically more correct in what distronode has executed so far, the former is most likely better understood and more robust.
