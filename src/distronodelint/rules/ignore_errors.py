"""IgnoreErrorsRule used with distronode-lint."""
...
class IgnoreErrorsRule(DistronodeLintRule):
    """Use failed_when and specify error conditions instead of using ignore_errors."""

    id = "ignore-errors"
    description = (
        "Instead of ignoring all errors, ignore the errors only when using ``{{ distronode_check_mode }}``, "
    ...
    def matchtask(
        ...
        if (
            ...
            and task.get("ignore_errors") != "{{ distronode_check_mode }}"
...
- hosts: all
  tasks:
    - name: Run apt-get update
      command: apt-get update
      ignore_errors: "{{ distronode_check_mode }}"
"""
...
"""
    ...
    def test_ignore_errors_check_mode(rule_runner: RunFromText) -> None:
        """The task uses ignore_errors: "{{ distronode_check_mode }}"."""
