...
from distronodelint.constants import LINE_NUMBER_KEY, SKIPPED_RULES_KEY
from distronodelint.file_utils import Lintable
from distronodelint.rules import DistronodeLintRule, TransformMixin
from distronodelint.yaml_utils import load_yamllint_config
...
class YamllintRule(DistronodeLintRule, TransformMixin):
...
def _combine_skip_rules(data: Any) -> set[str]:
    ...
    if tags and (
        ...
        and "skip_distronode_lint" in tags
        or tags == "skip_distronode_lint"
    ):
        result.add("skip_distronode_lint")
    return result
...
if "pytest" in sys.modules:
    ...
    @pytest.mark.filterwarnings("ignore::distronode_compat.runtime.DistronodeWarning")
    def test_yamllint(
        file: str,
        expected_kind: str,
        expected: list[str],
