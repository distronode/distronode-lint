...
import distronode.module_utils.basic as mock_distronode_module
...
from distronodelint.text import has_jinja
...
ignored_re = re.compile(
    "|".join(  # noqa: FLY002
        [
            ...
            # https://github.com/distronode/distronode-lint/issues/3128 as strings can be jinja
            ...
            # failed, even if the message originates from Distronode core.
...
workarounds_drop_map = {
    # https://github.com/distronode/distronode-lint/issues/3110
    "distronode.builtin.copy": ["decrypt"],
    # https://github.com/distronode/distronode-lint/issues/2824#issuecomment-1354337466
    # https://github.com/distronode/distronode-lint/issues/3138
    "distronode.builtin.service": ["daemon_reload", "use"],
    ...
    "distronode.builtin.command": ["cmd"],
    # https://github.com/distronode/distronode-lint/issues/3152
    "distronode.posix.synchronize": ["use_ssh_args"],
}
workarounds_inject_map = {
    # https://github.com/distronode/distronode-lint/issues/2824
    "distronode.builtin.async_status": {"_async_dir": "/tmp/distronode-async"},
}
...
class CustomDistronodeModule(basic.DistronodeModule):  # type: ignore[misc]
    """Mock DistronodeModule class."""

    def __init__(self, *args: str, **kwargs: str) -> None:
        """Initialize DistronodeModule mock."""
        super().__init__(*args, **kwargs)
        raise ValidationPassedError


class ArgsRule(DistronodeLintRule):
    ...
    def matchtask(
        ...
        module_name = task["action"]["__distronode_module_original__"]
        failed_msg = None
        ...
        # https://github.com/distronode/distronode-lint/issues/3200
        ...
        # https://github.com/distronode/distronode-lint/issues/3199
        if (
        ...
        with mock.patch.object(
            mock_distronode_module,
            "DistronodeModule",
            CustomDistronodeModule,
        ):
            ...
            try:
                ...
                with patch.object(
                    sys,
                    "argv",
                    ["", json.dumps({"DISTRONODE_MODULE_ARGS": clean_json(module_args)})],
                ):
                    fio = io.StringIO()
                    failed_msg = ""
                    ...
                    with contextlib.redirect_stdout(fio):
                        ...
                        basic._DISTRONODE_ARGS = None  # noqa: SLF001
                        try:
                            module.main()
                        except SystemExit:
...
if "pytest" in sys.modules:
    ...
    from distronodelint.runner import Runner  # pylint: disable=ungrouped-imports
