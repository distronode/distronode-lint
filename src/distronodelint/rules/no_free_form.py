...
from distronodelint.constants import INCLUSION_ACTION_NAMES, LINE_NUMBER_KEY
from distronodelint.rules import DistronodeLintRule, TransformMixin
from distronodelint.rules.key_order import task_property_sorter
...
class NoFreeFormRule(DistronodeLintRule, TransformMixin):
    ...
    def matchtask(
        ...
        action = task["action"]["__distronode_module_original__"]

        if action in INCLUSION_ACTION_NAMES:
            return results

        action_value = task["__raw_task__"].get(action, None)
        if task["action"].get("__distronode_module__", None) == "raw":
            ...
            fail = False
            if task["action"].get("__distronode_module__") in (
                "distronode.builtin.command",
                "distronode.builtin.shell",
                "distronode.windows.win_command",
