...
import distronodelint.yaml_utils
...
from distronodelint.config import PROFILES, Options
from distronodelint.config import options as default_options
from distronodelint.constants import LINE_NUMBER_KEY, RULE_DOC_URL, SKIPPED_RULES_KEY
from distronodelint.errors import MatchError
from distronodelint.file_utils import Lintable, expand_paths_vars
...
class DistronodeLintRule(BaseRule):
    """DistronodeLintRule should be used as base for writing new rules."""
    ...
    def _enrich_matcherror_with_task_details(
        match: MatchError,
        task: distronodelint.utils.Task,
    ) -> None:
        match.task = task
    ...
    def matchlines(self, file: Lintable) -> list[MatchError]:
        ...
        for prev_line_no, line in enumerate(file.content.split("\n")):
            ...
            rule_id_list = distronodelint.skip_utils.get_rule_skips_from_line(
                line,
                lintable=file,
            )
    ...
    def matchtasks(self, file: Lintable) -> list[MatchError]:
        ...
        for task in distronodelint.utils.task_in_list(
            data=file.data,
            kind=file.kind,
            file=file,
        ):
            if task.error is not None:
                # normalize_task converts DistronodeParserError to MatchError
                return [task.error]

            if (
                self.id in task.skip_tags
                or ("action" not in task.normalized_task)
                or "skip_distronode_lint" in task.normalized_task.get("tags", [])
            ):
                continue
    ...
    def matchyaml(self, file: Lintable) -> list[MatchError]:
        ...
        yaml = file.data
        # yaml returned can be an DistronodeUnicode (a string) when the yaml
        # file contains a single string. YAML spec allows this but we consider
        # this an fatal error.
        if isinstance(yaml, str):
            if yaml.startswith("$DISTRONODE_VAULT"):
                return []
        ...
        for play in yaml:
            ...
            if "skip_distronode_lint" in play.get("tags", []):
                continue
...
class TransformMixin:
    """A mixin for DistronodeLintRule to enable transforming files.

    If distronode-lint is started with the ``--fix`` option, then the ``Transformer``
...
def load_plugins(
    ...
    for rule in rules.values():  # type: ignore[assignment]
        if isinstance(rule, DistronodeLintRule) and bool(rule.id):
            yield rule
...
class RulesCollection:
    ...
    def register(self, obj: DistronodeLintRule, *, conditional: bool = False) -> None:
        """Register a rule."""
    ...
    def extend(self, more: list[DistronodeLintRule]) -> None:
        """Combine rules."""
    ...
    def list_tags(self) -> str:
        ...
        tag_desc = {
            ...
            "deprecations": "Indicate use of features that are removed from Distronode",
