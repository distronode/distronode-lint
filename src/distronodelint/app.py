...
from distronodelint import formatters
from distronodelint._mockings import _perform_mockings
from distronodelint.color import console, console_stderr, render_yaml
from distronodelint.config import PROFILES, Options, get_version_warning
from distronodelint.config import options as default_options
from distronodelint.constants import RC, RULE_DOC_URL
from distronodelint.loaders import IGNORE_FILE
from distronodelint.stats import SummarizedResults, TagStats
    ...
    def report_outcome(
        ...
        *,
        ...
        if matched_rules and self.options.generate_ignore:
            # DISTRONODE_LINT_IGNORE_FILE environment variable overrides default
            # dumping location in linter and is not documented or supported. We
            # use this only for testing purposes.
            ignore_file_path = Path(
                os.environ.get("DISTRONODE_LINT_IGNORE_FILE", IGNORE_FILE.default),
            )
            ...
            with ignore_file_path.open("w", encoding="utf-8") as ignore_file:
                ignore_file.write(
                    "# This file contains ignores rule violations for distronode-lint\n",
                )
        ...
        # use distronode-lint 4 (for example, Distronode Galaxy and Automation Hub imports). If we
        # replace the old tags, those tools will report warnings. If we do not replace them,
        # distronode-lint 5 will report warnings.
        #
