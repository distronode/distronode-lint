...
from distronode.parsing.yaml.constructor import DistronodeMapping
from distronode.plugins.loader import add_all_plugin_dirs
from distronode_compat.runtime import DistronodeWarning
...
from distronodelint.file_utils import Lintable, expand_dirs_in_lintables
from distronodelint.logger import timed_info
from distronodelint.rules.syntax_check import OUTPUT_PATTERNS
...
class Runner:
    ...
    def _update_exclude_paths(self, exclude_paths: list[str]) -> None:
        if exclude_paths:
            ...
            paths = distronodelint.file_utils.expand_paths_vars(exclude_paths)
            # Since distronodelint.utils.find_children returns absolute paths,
    ...
    def run(self) -> list[MatchError]:
        ...
        with warnings.catch_warnings(record=True) as captured_warnings:
            ...
            for warn in captured_warnings:
                # Silence Distronode runtime warnings that are unactionable
                # https://github.com/distronode/distronode-lint/issues/3216
                if warn.category is DistronodeWarning and isinstance(warn.source, dict):
                    msg = warn.source["msg"]
                    if msg.startswith(
                        "Falling back to Distronode unique filter as Jinja2 one failed",
                    ):
                        continue
                # For the moment we are ignoring deprecation warnings as Distronode
    ...
    def _run(self) -> list[MatchError]:
        ...
        # -- phase 2 ---
        if not matches:
            # do our processing only when distronode syntax check passed in order
            ...
            for file in self.lintables:
                ...
                _logger.debug(
                    ...
                    distronodelint.file_utils.normpath(file.path),
                    file.kind,
                )
    ...
    def _get_distronode_syntax_check_matches(
        self,
        lintable: Lintable,
        app: App,
    ) -> list[MatchError]:
        """Run distronode syntax check and return a list of MatchError(s)."""
        try:
...
"""
            ...
            # https://github.com/distronode/distronode-lint/issues/3650
            env["DISTRONODE_INVENTORY_ANY_UNPARSED_IS_FAILED"] = "False"
    ...
    def find_children(self, lintable: Lintable) -> list[Lintable]:
        ...
        distronodelint.utils.set_collections_basedir(lintable.path.parent)
        add_all_plugin_dirs(playbook_dir or ".")
        if lintable.kind == "role":
            playbook_ds = DistronodeMapping({"roles": [{"role": str(lintable.path)}]})
            ...
            try:
                playbook_ds = distronodelint.utils.parse_yaml_from_file(str(lintable.path))
            except DistronodeError as exc:
                raise SystemExit(exc) from exc
        results = []
        # playbook_ds can be an DistronodeUnicode string, which we consider invalid
        if isinstance(playbook_ds, str):
            raise MatchError(lintable=lintable, rule=self.rules["load-failure"])
        for item in distronodelint.utils.playbook_items(playbook_ds):
...
def get_matches(rules: RulesCollection, options: Options) -> LintResult:
    ...
    """
    lintables = distronodelint.utils.get_lintables(opts=options, args=options.lintables)
    ...
    runner = Runner(
        *lintables,
        ...
        _skip_distronode_syntax_check=options._skip_distronode_syntax_check,  # noqa: SLF001
    )
