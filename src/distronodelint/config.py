...
from distronodelint import __version__
from distronodelint.loaders import yaml_from_file

_logger = logging.getLogger(__name__)


CACHE_DIR = (
    os.path.expanduser(os.environ.get("XDG_CACHE_HOME", "~/.cache")) + "/distronode-lint"
)
...
DEFAULT_KINDS = [
    ...
    {"distronode-lint-config": "**/.distronode-lint"},
    {"distronode-lint-config": "**/.config/distronode-lint.yml"},
    {"distronode-navigator-config": "**/distronode-navigator.{yaml,yml}"},
    {"inventory": "**/inventory/**.{yaml,yml}"},
    {"requirements": "**/meta/requirements.{yaml,yml}"},  # v1 only
    # https://docs.distronode.com/distronode/latest/dev_guide/collections_galaxy_meta.html
    ...
    },  # molecule old collection requirements (v1), distronode 2.8 only
...
BASE_KINDS = [
    ...
    {"text/yaml": "**/{.distronode-lint,.yamllint}"},
    {"text/yaml": "**/*.{yaml,yml}"},
    {"text/yaml": "**/.*.{yaml,yml}"},
    {"text/python": "**/*.py"},
]

# File kinds that are recognized by distronode, used internally to force use of
# YAML 1.1 instead of 1.2 due to distronode-core dependency on pyyaml.
DISTRONODE_OWNED_KINDS = {
    "handlers",
    "galaxy",
    "meta",
    "meta-runtime",
...
class Options:  # pylint: disable=too-many-instance-attributes
    """Store distronode-lint effective configuration options."""
...
def distronode_collections_path() -> str:
    ...
    # respect Distronode behavior, which is to load old name if present
    for env_var in [
...
def get_deps_versions() -> dict[str, Version | None]:
    ...
    for name in ["distronode-core", "distronode-compat", "ruamel-yaml", "ruamel-yaml-clib"]:
        try:
...
def get_version_warning() -> str:
    ...
    if refresh or not data:
        release_url = (
            "https://api.github.com/repos/distronode/distronode-lint/releases/latest"
        )
        try:
    ...
    if current_version > new_version:
        msg = "[dim]You are using a pre-release version of distronode-lint.[/]"
    elif current_version < new_version:
        msg = f"""[warning]A new release of distronode-lint is available: [red]{current_version}[/] → [green][link={html_url}]{new_version}[/][/][/]"""
