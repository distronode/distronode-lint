...
from distronodelint.constants import CUSTOM_RULESDIR_ENVVAR, DEFAULT_RULESDIR, EPILOG, RC
from distronodelint.file_utils import (
    Lintable,
    abspath,
    expand_path_vars,
    find_project_root,
...
def get_config_path(config_file: str | None = None) -> str | None:
    ...
    if config_file:
        ...
        project_filenames = [
            ".distronode-lint",
            ".config/distronode-lint.yml",
            ".config/distronode-lint.yaml",
        ]
...
def get_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Allow distronode-lint to perform auto-fixes, including YAML reformatting. "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Generate a text file '.distronode-lint-ignore' that ignores all found violations. Each line contains filename and rule id separated by a space.",
    )
    parser.add_argument(
        "-w",
        "--warn-list",
    ...
    parser.add_argument(
        ...
        "--config-file",
        dest="config_file",
        help="Specify configuration file to use. By default it will look for '.distronode-lint', '.config/distronode-lint.yml', or '.config/distronode-lint.yaml'",
    )
    parser.add_argument(
        "-i",
        "--ignore-file",
