...
from distronodelint.config import DISTRONODE_OWNED_KINDS, BASE_KINDS, Options, options
from distronodelint.constants import CONFIG_FILENAMES, FileType, States
...
class Lintable:
        ...
        if self.kind == "yaml":
            if (
                ...
                and (
                    ...
                    or "distronode.builtin.import_playbook" in self.data[0]
                )
            ):
    ...
    def write(self, *, force: bool = False) -> None:
        ...
        if os.environ.get("DISTRONODE_LINT_WRITE_TMP", "0") == "1":
    ...
    def is_owned_by_distronode(self) -> bool:
        """Return true for YAML files that are managed by Distronode."""
        return self.kind in DISTRONODE_OWNED_KINDS
        ...
        if self.state == States.NOT_LOADED:
            ...
            try:
                if str(self.base_kind) == "text/yaml":
                    from distronodelint.utils import (  # pylint: disable=import-outside-toplevel
                    ...
                    if "append_skipped_rules" not in globals():
                        ...
                        from distronodelint.skip_utils import append_skipped_rules
...
def find_project_root(
    ...
    """Return a directory containing .git or distronode-lint config files.
