...
from distronodelint.yaml_utils import (
    FormattedYAML,
    get_path_to_play,
    get_path_to_task,
)
...
class Transformer:
    ...
    The Transformer is similar to the ``distronodelint.runner.Runner`` which manages
    ...
    def run(self) -> None:
        ...
        for file, matches in self.matches_per_file.items():
            ...
            if file_is_yaml:
                ...
                yaml = FormattedYAML(
                    # Distronode only uses YAML 1.1, but others files should use newer 1.2 (ruamel.yaml defaults to 1.2)
                    version=(1, 1)
                    if file.is_owned_by_distronode()
                    else None,
                )
    ...
    def _do_transforms(
        ...
        for match in sorted(matches):
            ...
            if self.write_set != {"all"}:
                rule = cast(DistronodeLintRule, match.rule)
